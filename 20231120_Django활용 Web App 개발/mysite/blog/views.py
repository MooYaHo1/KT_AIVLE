...
def list(request):
    ...
    return render(request, 'blog/list.html', {'post_all' : post_list, 'q':search_key}) # 템플릿을 응답하고 싶을 때 사용
...
def detail(request, no):
    ...
    tts.save("mysite/static/audio/hi.mp3")
    # 현재 작업중인 디렉토리에 저장된 hi.mp3 파일을 재생해보겠습니다.
    wn = Audio('mysite/static/audio/hi.mp3', autoplay=True)
    # display(wn)
    # return HttpResponse(post.title)
    comment_form=CommentForm()
    return render(request, 'blog/detail.html', {'post' : post, 'comment_all':comment_list, 'tag_list':tag_list, 'comment_form':comment_form})
